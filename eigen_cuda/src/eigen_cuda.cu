...
__global__ void eigenGPU(float* neighbor_points,float* eigen_vecotr,float* eigen_value,int point_size) {
    //平均計算
    float x_average=0,y_average=0,z_average=0;
    for(int i=0;i<point_size*3;i+=3){
        x_average+=neighbor_points[i];
    ...
    eigenJacobiMethod(a, eigen_vecotr, 3);
    eigen_value[0]=a[0];
    eigen_value[1]=a[4];
    eigen_value[2]=a[8];
...
extern void eigen(std::vector<std::vector<float>> neighbor_points,float eigen_vecotr[3][3],float eigen_value[3]){
    //変数宣言
    std::vector<float> h_neighbor_points(neighbor_points.size() * 3);
    std::vector<float> h_eigen_vector(3 * 3);
    std::vector<float> h_eigen_value(3);
    float *d_neighbor_points, *d_eigen_vecotr, *d_eigen_value;

    //メモリ確保
    cudaMalloc((void **)&d_neighbor_points, neighbor_points.size() * 3 * sizeof(float));
    cudaMalloc((void **)&d_eigen_vecotr, 3 * 3 * sizeof(float));
    ...
    eigenGPU<<<1, 1>>>(d_neighbor_points,d_eigen_vecotr,d_eigen_value,neighbor_points.size());

    //配列にコピー
    cudaMemcpy(&h_eigen_vector[0], d_eigen_vecotr, 3 * 3 * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            eigen_vecotr[i][j]=h_eigen_vector[k];
